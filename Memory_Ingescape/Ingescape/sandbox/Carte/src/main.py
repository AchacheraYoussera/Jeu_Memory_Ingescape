...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Carte)
        print("event received: ", event, " uuid: ", uuid, " name: ", name, " event_data: ", event_data)
...
# attributes
def Valeur_attribute_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Carte)
        agent_object.ValeurA = value
        # add code here if needed
    except:
        print(traceback.format_exc())


def Nom_attribute_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Carte)
        agent_object.NomA = value
        # add code here if needed
    except:
        print(traceback.format_exc())


def Pouvoir_attribute_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Carte)
        agent_object.PouvoirA = value
        # add code here if needed
    except:
        print(traceback.format_exc())


def Jou__E_attribute_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Carte)
        agent_object.Jou__EA = value
        # add code here if needed
    except:
        print(traceback.format_exc())


def Sortie_attribute_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Carte)
        agent_object.SortieA = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.attribute_create("Valeur", igs.INTEGER_T, None)
    igs.attribute_create("Nom", igs.STRING_T, None)
    igs.attribute_create("Pouvoir", igs.STRING_T, None)
    igs.attribute_create("Jouée", igs.BOOL_T, None)
    igs.attribute_create("Sortie", igs.BOOL_T, None)

    igs.observe_attribute("Valeur", Valeur_attribute_callback, agent)
    igs.observe_attribute("Nom", Nom_attribute_callback, agent)
    igs.observe_attribute("Pouvoir", Pouvoir_attribute_callback, agent)
    igs.observe_attribute("Jouée", Jou__E_attribute_callback, agent)
    igs.observe_attribute("Sortie", Sortie_attribute_callback, agent)
